  ...
  try {
    const [images] = await db.promise().query(
      `SELECT id, title, description, image_filename, \`order\`, active, created_at
       FROM gallery_images ORDER BY \`order\` ASC, id DESC`
    );
...
// GET images for a specific page (public / frontend use)
...
router.post('/', protect, upload.single('image'), async (req, res) => {
  ...
  try {
    const [result] = await db.promise().query(
      `INSERT INTO gallery_images (title, description, image_filename, \`order\`, active)
       VALUES (?, ?, ?, ?, ?)`,
    ...
    if (pages.length > 0) {
      ...
      await db.promise().query(
        `INSERT INTO gallery_image_pages (image_id, page_slug) VALUES ?`,
        [values]
      );
    }
...
router.put('/:id', protect, upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    // Update assigned pages (replace all)
    await db.promise().query(`DELETE FROM gallery_image_pages WHERE image_id = ?`, [id]);
...
router.delete('/:id', protect, async (req, res) => {
  const { id } = req.params;
  try {
    await db.promise().query(`DELETE FROM gallery_images WHERE id = ?`, [id]);
    // junction rows auto-deleted via foreign key ON DELETE CASCADE
